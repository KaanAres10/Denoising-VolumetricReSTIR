...
# CONFIG
# ----------------------------
PIPE_MODE    = "NODENOISER"   # "OIDN" | "NODENOISER" | "REF"
CAPTURE_MODE = "frames"       # "timing" | "frames" | "both"

OIDN_MODE = "Fast"            # "Fast" | "Balanced" | "High"
...
OUT_ROOT = os.path.join(
    SCRIPT_DIR,
    "outputs",
    "Plume_OIDN_Improved_NoDenoiser"
)


if PIPE_MODE == "OIDN":
    OUT_DIR = os.path.join(OUT_ROOT, f"{OIDN_MODE}")
    BASE    = f"oidn_{OIDN_MODE.lower()}"
elif PIPE_MODE == "NODENOISER":
    ...
    raise RuntimeError("PIPE_MODE must be one of: OIDN | NODENOISER | REF")
...
def render_graph():
    ...
    # Shared path for OIDN / NODENOISER
