...
public:
    ...
    __hostdev__ uint32_t flags() const { return DataType::mFlags; }

    __hostdev__ int32_t offset() const { return DataType::mOffset; }

    //__hostdev__ void copyMTable(DataType::Tile* dst, uint32_t offset) const { memcpy(dst + offset, DataType::mTable, (1u << (3 * LOG2DIM)) * sizeof(DataType::Tile)); }

    __hostdev__ uint32_t getChild(uint32_t n) const { return DataType::mTable[n].childID; }
...
public:
    ...
    __hostdev__ CoordT get_BBox_min() const
    {
        return DataType::mBBoxMin;
    }

    __hostdev__ void get_BBoxDif(uint8_t ret[3]) const
    {
        ret[0] = DataType::mBBoxDif[0];
        ret[1] = DataType::mBBoxDif[1];
        ret[2] = DataType::mBBoxDif[2];
    }
