...
{
    gOutSpecularIllumination[ipos] = float4(1, 1, 1, 1);
    gOutDiffuseIllumination[ipos] = float4(1, 1, 1, 1);
    return;
    ...
    // Check 1: Is the shader running? (Output WHITE)
    gOutDiffuseIllumination[ipos] = float4(1, 1, 1, 1); return;

    // Check 2: Is Input Diffuse valid? (Output Raw Input)
    // gOutDiffuseIllumination[ipos] = gDiffuseIllumination[ipos]; return;

    // Check 3: Is ViewZ valid? (Output Green if Z > 0)
    // float zCheck = gViewZ[ipos];
    // if (zCheck > 0.0 && zCheck < 50000.0) gOutDiffuseIllumination[ipos] = float4(0, 1, 0, 1); 
    // else gOutDiffuseIllumination[ipos] = float4(1, 0, 0, 1); // Red = Bad Z
    // return;
