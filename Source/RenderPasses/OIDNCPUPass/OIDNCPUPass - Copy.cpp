...
// Reuse the same shader files OptixDenoiser uses:
...
// Allocate / reallocate GPU staging buffers
    ...
    Buffer::SharedPtr& outBufCPU)
{
    ...
    // GPU buffers: UAV + SRV for compute/pixel shaders, NO CPU access
    ...
    // CPU staging buffers: CPU readable/writable, NO UAV/RT
...
// execute(): 
//   src texture -> convertTexToBuf (GPU) -> mInputBufGPU
//   mInputBufGPU (map) -> OIDN (CPU) -> mOutputBufGPU (map)
//   mOutputBufGPU -> convertBufToTex (GPU) -> dst texture
void OIDNCPUPass::execute(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    // --------------------------------------------------------------------
    // 1) GPU: texture -> GPU input buffer via ConvertTexToBuf
    // --------------------------------------------------------------------
    {
    ...
    // --------------------------------------------------------------------
    {
        ...
        const size_t pixelStride = sizeof(float) * 4; // RGBA
        const size_t rowStride = 0;                 // tightly packed rows
        ...
        // pick HDR/sRGB correctly for your data
        ...
        if (err != oidn::Error::None)
        {
            ...
            // fallback to noisy input instead of black
        ...
        // Write OIDN result into CPU output buffer
    ...
    // --------------------------------------------------------------------
    // 4) GPU: CPU output buffer -> GPU output buffer
    // --------------------------------------------------------------------
    pRenderContext->copyResource(mOutputBufGPU.get(), mOutputBufCPU.get());

    // --------------------------------------------------------------------
    // 5) GPU: GPU output buffer -> texture via ConvertBufToTex
    // --------------------------------------------------------------------
    {
